...
public class Etc {
	...
	public static void main(String[] args) {
		/*
		 * <<비트 연산자>>
		 * |(OR)      : 피연산자 중 한 쪽이라도 값이 1이면, 1을그외는 0을 결과로 얻는다.
		 * &(AND)     : 피연산자 양 쪽이 모두 1이어야만 1을, 그외는 0을 결과로 얻는다.
		 * ^(XOR)     : 피연산자의 값이 서로 다를때만 1을, 같을 때는 0을 결과로 얻는다.
		 * ~(비트전환)  : 피연산자를 0은 1로, 1은 0으로 바꾼다.
		 * <<(쉬프트)   : 피연산자의 각 자리를 왼쪽으로 이동시킨다. 빈칸은 0으로 채운다.
		 * >>(쉬프트)   : 피연산자의 각 자리를 오른쪽으로 이동시킨다. 빈칸은 옆자리와 동일한 값으로 채운다.
		 * 
		 * <<기타 연산자>>
		...
		// |(OR) : 00001111 //둘중 하나라도 1이면 1이니까 10과 15를 비교하면 15가 나옴
		System.out.println(10 | 15);
		// & : 00001010 //둘 다 1이여야 1이니까 10이 됨
		System.out.println(10 & 15);
		//^ : 00000101 // 두 값이 다를때 1
		System.out.println(10 ^ 15);
		//~ : 11110101 (00001010)
		System.out.println(~10);
		//2진수의 맨 앞자리가 1이면 음수, 0이면 양수
		//<< : 00010100 (00001010)
		System.out.println(10 << 1); //'10을 왼쪽으로 1칸 이동시켰다' 라는 뜻
		//>> : 00000101 (00001010)
		...
		// int result = true ? x : y;
		// int result = x; // 연산결과
		System.out.println(result); //참이니까 x값이 result에 저장
		
		//정수가 60점 이상이면 합격 미만이면 불합격
		int score = 60;
		String res = 60 <= score ?"합격" :"불합격";
		System.out.println(res);
		
		//주민등록번호 뒷자리의 첫번째숫자가 1이면 남자 2면 여자
		...
		//변수에 저장된 수의 절대값을 출력해주세요.  //abs (absolute value)
		int i = -5;
		...
		regNo = 1;
		String gen = (regNo == 1 || regNo == 3) ?"남자" : regNo == 2 || regNo == 4 ?"여자" :"확인불가";
