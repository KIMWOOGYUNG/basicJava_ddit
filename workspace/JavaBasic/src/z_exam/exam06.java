	...
	public static void main(String[] args) {
		...
		[6-2] 문제6-1에서 정의한 SutdaCard클래스에 두 개의 생성자와 info()를 추가해서 실행결과와 같은 결과를 얻도록 하시오.
			...
			class SutdaCard { 
				...
				//생성자1
				public SutdaCard(int num, boolean isKwang){ //타입을 설정
					this.num = Numb; //지역변수가 이름이 같을때 구분하기 위해 붙임 / 매개변수로 받을때
				...
				public String info(){ //출력하기 위해 만든것
		...
		/*[6-9] 다음은 컴퓨터 게임의 병사(marine)를 클래스로 정의한 것이다. 
			...
			해설 : 모두 같아야하는 변수는 스태틱 붙여야함
			  	   스태틱 변수에 대한 메서드도 스태틱 붙여야함
			*/
			
		/*[6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은? (모두 고르시오) //b,e
				a. 모든 생성자의 이름은 클래스의 이름과 동일해야한다.
				b. 생성자는 객체를 생성하기 위한 것이다. //객체를 초기화하기 위해 (생성만 하는건 new연산자)
				c. 클래스에는 생성자가 반드시 하나 이상 있어야 한다.
				d. 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다.
				e. 생성자는 오버로딩 할 수 없다. //생성자도 오버로딩 가능, 한 클래스 내에 여러개의 생성자 정의할 수 있음
					
		[6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오) //b
				a. 객체 자신을 가리키는 참조변수이다.
				b. 클래스 내에서라면 어디서든 사용할 수 있다. //this는 인스턴스 자신의 주소를 저장, 클래스멤버에서는 사용불가
				c. 지역변수와 인스턴스변수를 구별할 때 사용한다.
				d. 클래스 메서드 내에서는 사용할 수 없다. //인스턴스 메서드만
					
		[6-12] 다음 중 오버로딩이 성립하기 위한 조건이 아닌 것은? (모두 고르시오) //c,d
				a. 메서드의 이름이 같아야 한다.
				b. 매개변수의 개수나 타입이 달라야 한다.
				c. 리턴타입이 달라야 한다. //동일
				d. 매개변수의 이름이 달라야 한다. //동일
				
		[6-13] 다음 중 아래의 add메서드를 올바르게 오버로딩 한 것은? (모두 고르시오) //b,c,d
				long add(int a, int b) { return a+b;}
				a. long add(int x, int y) { return x+y;} //매개변수의 이름은 동일해야함
		...
		[6-14] 다음 중 초기화에 대한 설명으로 옳지 않은 것은? (모두 고르시오) //c,e
				...
				c.초기화 블럭보다 생성자가 먼저 수행된다. //명시적->초기화블럭->생성자 순서
				d.명시적 초기화를 제일 우선적으로 고려해야 한다.
				e.클래스변수보다 인스턴스변수가 먼저 초기화된다. //클래스변수는 메모리에 올라갈때 자동으로 되기때문에 더 먼저 초기화됨
		...
		[6-16] 다음 중 지역변수에 대한 설명으로 옳지 않은 것은? (모두 고르시오) //a,e
				a. 자동 초기화되므로 별도의 초기화가 필요없다. //전역변수가 자동초기화
				b. 지역변수가 선언된 메서드가 종료되면 지역변수도 함께 소멸된다.
				c. 매서드의 매개변수로 선언된 변수도 지역변수이다.
				d. 클래스변수나 인스턴스변수보다 메모리 부담이 적다.
				e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다. //지역변수는 호출스택에 생성 / 힙에는 인스턴스변수 생성	
		...
		[6-17] 호출스택이 다음과 같은 상황일 때 옳지 않은 설명은? (모두 고르시오) //b
				...
				b. println메서드를 제외한 나머지 메서드들은 모두 종료된 상태이다. //종료가 아니라 대기상태
		...
		/*[6-18] 다음의 코드를 컴파일하면 에러가 발생한다. 컴파일 에러가 발생하는 라인과 그 이유를 설명하시오.
				class MemberCall {
					...
					static int cv = 20;
					int iv2 = cv;
					static int cv2 = iv; // 라인 A //스태틱 변수 초기화에 인스턴스 변수 사용x
					
					static void staticMethod1() {
						System.out.println(cv);
						System.out.println(iv); // 라인 B //스태틱 메서드에서 인스턴스 변수 사용x
					}
					...
					static void staticMethod2() {
						staticMethod1();
						instanceMethod1(); // 라인 D //스태틱 메서드에서 인스턴스 메서드 사용x
					}
		...
		//스트링클래스는 배열크기가 정해져있기 때문에 변경이 가해지면 새로운 객체를 만듬
...
*/		
		...
		/*[6-20] 다음과 같이 정의된 메서드를 작성하고 테스트하시오.
			...
			class Exercise6_20
			{
				public static int[] shuffle(int[] arr) { if(arr==null || arr.length==0) return arr;
