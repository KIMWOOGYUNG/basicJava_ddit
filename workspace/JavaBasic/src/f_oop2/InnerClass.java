...
//인스턴스 클래스---------------------------------------------------
	class InstanceInner{
		...
		void method(){
			...
			System.out.println(this.var); //인스턴스 변수이자 전역변수
			System.out.println(InnerClass.this.var); //밖에있는 클래스에 접근
...
//스태틱 클래스----------------------------------------------
	static class StaticInner{
		...
		//int inner1 = instanceVar; //인스턴스 멤버라 오류
		int inner2 = classVar;
...
//지역클래스------------------------------------------------
	void method(){
		...
		class LocalInner{
			...
			int inner1 = instanceVar;
			int inner2 = classVar;
//			int inner3 = localVar1; //final이 붙지 않은 지역 변수는 사용불가
