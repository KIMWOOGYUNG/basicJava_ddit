...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * - 부모 클래스의 멤버와 자식 클래스의 멤버의 이름이 중복될때 구분하기 위해 사용한다.
		 * 
		 * << super() >>
...
//Singleton-----------------------------------------------------------------------------------------------		
		Singleton s = Singleton.getInstance(); //싱글톤 클래스의 getInstance 메서드를 호출해서 변수s에 저장
											   //널일때 한번만 객체가 생성되니까 아무리 여러번 받아도 똑같은 객체
		
		System.out.println(new Inheritance());
		System.out.println(new Inheritance()); //두 주소가 다름
		
		System.out.println(Singleton.getInstance());
		System.out.println(Singleton.getInstance()); //동일 객체이므로 주소가 같음
	}
